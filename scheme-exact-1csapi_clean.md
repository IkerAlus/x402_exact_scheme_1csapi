...
The key architectural insight of this scheme is that the `payTo` field in `PaymentRequirements` is set to the **1Click Swap API deposit address** — a unique, one-time address generated by this service when the resource server requests a quote (`POST /v0/quote` with `dry: false`). This means:
...
### 6.2 Deposit Address Validity Window
...
### 6.4 Deposit Address Authenticity
