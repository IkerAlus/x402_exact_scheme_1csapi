...
The key architectural insight of this scheme is that the `payTo` field in `PaymentRequirements` is set to the **1Click Swap API deposit address** — a unique, one-time address generated by the 1Click backend when the resource server requests a quote (`POST /v0/quote` with `dry: false`). This means:
